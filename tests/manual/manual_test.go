...
func TestHackerOneAPI(t *testing.T) {
	// This test requires a real HackerOne API key
	// Set HACKERONE_API_KEY environment variable before running
	
	cfg, err := config.Load()
	require.NoError(t, err)
	
	if cfg.APIs.HackerOne.APIKey == "" {
		t.Skip("HACKERONE_API_KEY not set, skipping manual test")
	}
