...
func TestAssetRepository_CreateAssetResponse(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	mock.ExpectExec("INSERT INTO asset_responses").
		WithArgs(sqlmock.AnyArg(), assetResponse.AssetID, assetResponse.StatusCode, assetResponse.Headers, assetResponse.Body, assetResponse.ResponseTime, sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))

	err := repo.CreateAssetResponse(ctx, assetResponse)
	assert.NoError(t, err)
	assert.NotEqual(t, uuid.Nil, assetResponse.ID)
	assert.False(t, assetResponse.CreatedAt.IsZero())
}

func TestAssetRepository_GetAssetResponsesByAssetID(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	assetID := uuid.New()
	expectedResponses := []*AssetResponse{
		{
			ID:           uuid.New(),
			AssetID:      assetID,
			StatusCode:   200,
			Headers:      `{"Server": "nginx"}`,
			Body:         "<html>Response 1</html>",
			ResponseTime: 100,
			CreatedAt:    time.Now(),
		},
		{
			...
			Body:         "<html>Response 2</html>",
			ResponseTime: 120,
			CreatedAt:    time.Now(),
		},
	}

	rows := sqlmock.NewRows([]string{"id", "asset_id", "status_code", "headers", "body", "response_time", "created_at"})
	for _, resp := range expectedResponses {
		rows.AddRow(resp.ID, resp.AssetID, resp.StatusCode, resp.Headers, resp.Body, resp.ResponseTime, resp.CreatedAt)
	}

	mock.ExpectQuery("SELECT \\* FROM asset_responses WHERE asset_id = \\$1 ORDER BY created_at DESC").
		WithArgs(assetID).
		WillReturnRows(rows)

	responses, err := repo.GetAssetResponsesByAssetID(ctx, assetID)
	assert.NoError(t, err)
	assert.Len(t, responses, 2)
	assert.Equal(t, expectedResponses[0].ID, responses[0].ID)
	assert.Equal(t, expectedResponses[1].ID, responses[1].ID)
}

func TestAssetRepository_GetLatestAssetResponseByAssetID(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	assetID := uuid.New()
	expectedResponse := &AssetResponse{
		ID:           uuid.New(),
		AssetID:      assetID,
		StatusCode:   200,
		Headers:      `{"Server": "nginx"}`,
		Body:         "<html>Latest Response</html>",
		ResponseTime: 100,
		CreatedAt:    time.Now(),
	}

	rows := sqlmock.NewRows([]string{"id", "asset_id", "status_code", "headers", "body", "response_time", "created_at"}).
		AddRow(expectedResponse.ID, expectedResponse.AssetID, expectedResponse.StatusCode, expectedResponse.Headers, expectedResponse.Body, expectedResponse.ResponseTime, expectedResponse.CreatedAt)

	mock.ExpectQuery("SELECT \\* FROM asset_responses WHERE asset_id = \\$1 ORDER BY created_at DESC LIMIT 1").
		WithArgs(assetID).
		WillReturnRows(rows)

	response, err := repo.GetLatestAssetResponseByAssetID(ctx, assetID)
	assert.NoError(t, err)
	assert.Equal(t, expectedResponse, response)
}

func TestAssetRepository_SearchAssetResponsesByHeaders(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	mock.ExpectQuery("SELECT \\* FROM asset_responses WHERE headers ILIKE \\$1 ORDER BY created_at DESC").
		WithArgs("%" + searchPattern + "%").
		WillReturnRows(rows)

	responses, err := repo.SearchAssetResponsesByHeaders(ctx, searchPattern)
	assert.NoError(t, err)
	assert.Len(t, responses, 1)
	assert.Equal(t, expectedResponses[0].ID, responses[0].ID)
}

func TestAssetRepository_SearchAssetResponsesByBody(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	expectedResponses := []*AssetResponse{
		{
			...
			Body:         "<html><title>WordPress Site</title></html>",
			ResponseTime: 100,
			CreatedAt:    time.Now(),
		},
	}
	...
	mock.ExpectQuery("SELECT \\* FROM asset_responses WHERE body ILIKE \\$1 ORDER BY created_at DESC").
		WithArgs("%" + searchPattern + "%").
		WillReturnRows(rows)

	responses, err := repo.SearchAssetResponsesByBody(ctx, searchPattern)
	assert.NoError(t, err)
	assert.Len(t, responses, 1)
	assert.Equal(t, expectedResponses[0].ID, responses[0].ID)
}

func TestAssetRepository_SearchAssetResponsesByStatusCode(t *testing.T) {
	db, mock, cleanup := setupMockDB(t)
	defer cleanup()
	...
	mock.ExpectQuery("SELECT \\* FROM asset_responses WHERE status_code = \\$1 ORDER BY created_at DESC").
		WithArgs(statusCode).
		WillReturnRows(rows)

	responses, err := repo.SearchAssetResponsesByStatusCode(ctx, statusCode)
	assert.NoError(t, err)
	assert.Len(t, responses, 1)
	assert.Equal(t, expectedResponses[0].ID, responses[0].ID)
	assert.Equal(t, statusCode, responses[0].StatusCode)
}
